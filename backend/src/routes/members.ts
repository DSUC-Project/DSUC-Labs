...
router.put('/:id', authenticateWallet, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { name, avatar, skills, socials, bank_info } = req.body;
...
router.patch('/:id/role', authenticateWallet, requireAdmin, async (req: AuthRequest, res: Response) => {
  return res.status(403).json({
    error: 'Forbidden',
    message: 'Role changes are not allowed through API. Please access database directly.',
  });
});
